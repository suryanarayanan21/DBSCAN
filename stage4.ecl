...
l_stage3 := RECORD
  UNSIGNED4 nodeID;   //the node that the record locate
  UNSIGNED4 id;       //recordID
  UNSIGNED4 parentid; //parentID
  BOOLEAN if_local ; //local or remote
  BOOLEAN if_core ;  //core points or non-core.  Non_core can also be an outlier.
END;
...
STREAMED DATASET(l_stage3) ultimate(STREAMED DATASET(l_stage3) dsin, UNSIGNED4 pointcount) := EMBED(C++:activity)
...
// initial0 := IF(thorlib.node() = 0, ultimate(locals, c), ultimate(locals, 0));
// initial0 := IF(thorlib.node() = 0, ultimate(locals, c), ultimate(locals, 0));
...
// initial0 := ultimate(locals, c)[1..c];



// layout := RECORD
//   INTEGER id;
//   INTEGER parentID; //largestID
//   INTEGER ultimateID := -1; //ultimateID
// END;
layout := RECORD
  INTEGER id;
  INTEGER parentID;
...
ds2 := PROJECT(ds1, TRANSFORM(l_stage3,
                        SELF.nodeid := 0;
                        SELF.if_local := FALSE,
                        SELF.if_core := FALSE,
                        SELF := LEFT), LOCAL);
